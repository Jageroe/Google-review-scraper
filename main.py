...
def scrape_a_store(object_url: str) -> tuple :
    """
    this function will:
...
def extraxt_reviews(reviews_source: list) -> list:
...
def main():
    ...
    for i, url in enumerate(OBJECT_URLS):
        try:
            ...
            review_list = extraxt_reviews(reviews_source)
            scraped_data[i]['reviews'] = review_list

            if scraped_data[i]['cnt_reviews'] != len(scraped_data[i]['reviews']):
    ...
    # reorder the columns
    result_df = result_df[[
                'object_name','object_address','overall_rating','cnt_reviews',
                'object_url', 'name','date','rate','review','reply'
                ]]
